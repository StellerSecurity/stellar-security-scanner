...
LIMITS = {'archive_bytes': 256 * 1024 * 1024, 'text_bytes': 8 * 1024 * 1024,
...
class Inspector:
    ...
    def check_budget(self):
        ...
        if self.members_seen > self.limits['members']:
    ...
    def finding(self, path, rule, severity, malware=False):
        ...
        if len(self.findings) >= self.limits['findings']:
            raise Incomplete('finding-report-limit')
        self._findings_seen.add(key)
        self.findings.append({'path': path, 'rule': rule, 'severity': severity,
    ...
    def archive(self, path, raw, depth=0):
        ...
        def member_name(name, directory=False):
            ...
            if not directory and any(p.startswith(canonical + '/') for p in seen):
                raise Incomplete('archive-file-directory-conflict')
            seen[canonical] = 'directory' if directory else 'file'
            ...
            if raw.startswith((b'PK\x03\x04', b'PK\x05\x06')):
                ...
                with zipfile.ZipFile(io.BytesIO(raw)) as arc:
                    ...
                    for info in arc.infolist():
                        self.check_budget()
                        name = member_name(info.filename, info.is_dir())
                ...
                with tarfile.open(fileobj=decoded, mode='r|') as arc:
                    for info in arc:
                        self.check_budget()
                        name = member_name(info.name, info.isdir())
                        if info.isdir():
                            continue
    ...
    def report(self, source_sha, kind, archive_sha=None):
        ...
        blocked = any(x['severity'] in ('error', 'review') for x in self.findings)
